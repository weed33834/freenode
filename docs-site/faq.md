...
可以。`web/` 目录是一个 Next.js 应用，默认以 standalone 模式构建。运行 `cd web && npm install && npm run build` 后产物在 `web/dist/standalone`，可用 `node server.js` 启动，也可通过仓库自带的 `backend/docker-compose.yml` 一键部署（Caddy 反代 + 自动 HTTPS）。
...
- HTTP代理：`{{github_url}}/nodes/proxies.txt`
