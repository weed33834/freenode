...
| `mahdibland-eternity` | GitHub Raw | daily | vmess, vless, ss, trojan | 每日自动合并的公开节点集合（Base64） |
...
上表仅列出部分代表性节点源。当前共启用约 **28** 个节点源，完整清单请查看仓库中的 [`config/sources.json`](https://github.com/MS33834/freenode/blob/main/config/sources.json)。
...
> 另有 `gfpcom` 系列等大型代理列表默认禁用，可在本地按需开启。
